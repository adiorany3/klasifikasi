...
def calculate_correlations(df, selected_features):
    correlations = pd.DataFrame({
        ...
        'Correlation with IP_actual': [df[feature].corr(df['IP_actual']) 
                                     for feature in selected_features]
    }).sort_values('Correlation with IP_actual', key=abs, ascending=False)
    return correlations
...
def main():
    ...
    # Selected features
    selected_features = [
        'Age', 'Feed', 'Feed Intake', 'FI_actual', 'Culling', 'Death', 
        'Depletion', 'Livebird_after', '%Live', 
        'Live Weight / bird', 'ADG', 'FCR', 'value_calibration_temp', 
        'value_calibration_hum', 'THI', 'WCI'
    ]
