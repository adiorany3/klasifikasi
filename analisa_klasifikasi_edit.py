...
def process_features(df, selected_features):
    ...
    X_scaled_numeric = scaler.fit_transform(X)
    X_scaled = pd.DataFrame(X_scaled_numeric, columns=X.columns)
...
def main():
    ...
    if 'BIP' not in df.columns:
        st.error("The target column 'BIP' is not found in your data. Please make sure your Excel file contains a column named 'BIP'.")
        st.stop()

    # Only allow numeric columns for feature selection
    st.subheader("Select Numeric Features for Analysis")
    numeric_features = [col for col in df.columns if col != 'BIP' and pd.api.types.is_numeric_dtype(df[col])]
    if not numeric_features:
        st.error("No numeric features found in your data (besides BIP). Please check your file.")
        st.stop()
    selected_features = st.multiselect(
        "Choose numeric features to use for classification:",
        options=numeric_features,
        default=numeric_features
    )

    if not selected_features:
        st.error("Please select at least one numeric feature for analysis.")
        return

    # Process features with caching (only numeric columns)
    with st.spinner('Processing features...'):
        X_scaled, y_encoded, y_cat, le = process_features(df, selected_features)

    # Calculate correlations with caching (only numeric columns)
    ...
    # Add a model selection dropdown to train only selected models
    st.subheader("Select Models for Training")
    selected_models = st.multiselect(
        "Choose models to train:",
        options=list(model_options.keys()),
        default=['Random Forest', 'XGBoost']  # Default to faster models
    )
    ...
    # Add toggles for visualization options
    ...
    show_correlation_matrix = st.sidebar.checkbox("Show Correlation Matrix", value=True)
